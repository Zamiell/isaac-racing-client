...
function ChangeKeybindings:PostRender()
  ...
  local hotkeyAutofire = RacingPlusData:Get("hotkeyAutofire")
  ...
  if ChangeKeybindings.challengeState == ChangeKeybindings.states.FAST_DROP then
    if hotkeyDrop == 0 then
      ...
      text[2] = (
        ChangeKeybindings:GetKeyName(hotkeyDrop) .. " (code: " .. tostring(hotkeyDrop) .. ")"
      )
    end
  elseif ChangeKeybindings.challengeState == ChangeKeybindings.states.FAST_DROP_TRINKET then
    if hotkeyDropTrinket == 0 then
      ...
      text[2] = (
        ChangeKeybindings:GetKeyName(hotkeyDropTrinket)
  ...
  elseif ChangeKeybindings.challengeState == ChangeKeybindings.states.FAST_DROP_POCKET then
    if hotkeyDropPocket == 0 then
      ...
      text[2] = (
        ChangeKeybindings:GetKeyName(hotkeyDropPocket)
  ...
  elseif ChangeKeybindings.challengeState == ChangeKeybindings.states.SCHOOLBAG_SWITCH then
    if hotkeySwitch == 0 then
      ...
      text[2] = (
        ChangeKeybindings:GetKeyName(hotkeySwitch)
        .. " (code: " .. tostring(hotkeySwitch) .. ")"
      )
    end
  elseif ChangeKeybindings.challengeState == ChangeKeybindings.states.AUTOFIRE then
    if hotkeyAutofire == 0 then
      text[1] = "The autofire hotkey is not bound."
    else
      text[1] = "The autofire hotkey is currently bound to:"
      text[2] = (
        ChangeKeybindings:GetKeyName(hotkeyAutofire)
        .. " (code: " .. tostring(hotkeyAutofire) .. ")"
      )
    end
  end
  if text[2] == nil then
  ...
  for k, v in pairs(Keyboard) do
    if Input.IsButtonPressed(v, 0) then
      ...
      elseif ChangeKeybindings.challengeState == ChangeKeybindings.states.SCHOOLBAG_SWITCH then
        ...
        ChangeKeybindings.challengeState = ChangeKeybindings.states.AUTOFIRE
        ChangeKeybindings.challengeFramePressed = gameFrameCount
      elseif ChangeKeybindings.challengeState == ChangeKeybindings.states.AUTOFIRE then
        if v == Keyboard.KEY_F12 then -- 301
          v = 0
        end
        RacingPlusData:Set("hotkeyAutofire", v)
