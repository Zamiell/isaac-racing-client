...
function RacePostUpdate:Check3DollarBill()
  if (
    ...
    and g.race.rFormat == "seeded"
    and g.p:HasCollectible(CollectibleType.COLLECTIBLE_3_DOLLAR_BILL) -- 191
  ) then
    g.p:RemoveCollectible(CollectibleType.COLLECTIBLE_3_DOLLAR_BILL) -- 191
    Isaac.DebugString(
      "Removing collectible " .. tostring(CollectibleType.COLLECTIBLE_3_DOLLAR_BILL) -- 191
    )
...
function RacePostUpdate:CheckFireworks()
  ...
  if (
    Isaac.CountEntities(nil, EntityType.ENTITY_EFFECT, EffectVariant.FIREWORKS, -1) > 0 -- 1000.104
    and g.sfx:IsPlaying(SoundEffect.SOUND_BOSS1_EXPLOSIONS) -- 182
  ) then
  ...
  if (
    (
      ...
      and g.race.status == "none"
      and g.race.place == 1
      and g.race.numEntrants >= 3
    )
    or Speedrun.finished
  ) then
    -- Give Isaac sparkly feet (1000.103.0)
    local blingPos = g.p.Position + RandomVector() * 10
    Isaac.Spawn(
    ...
    if g.raceVars.fireworks < 40 and gameFrameCount % 20 == 0 then
      for i = 1, 5 do
        ...
        local fireworkPos = g:GridToPos(math.random(1, 11), math.random(2, 8))
        local firework = Isaac.Spawn(
...
function RacePostUpdate:CheckKeeperHolyMantle()
  ...
  if (
    g.run.tempHolyMantle
    and not effects:HasCollectibleEffect(CollectibleType.COLLECTIBLE_HOLY_MANTLE) -- 313
  ) then
...
function RacePostUpdate:CheckFinalButtons(gridEntity, i)
  ...
  for _, button in ipairs(g.run.buttons) do
    if button.type == "victory-lap" and button.roomIndex == roomIndex then
      if (
        gridEntity:GetSaveState().State == 3
        and gridEntity.Position.X == button.pos.X
        and gridEntity.Position.Y == button.pos.Y
      ) then
    ...
    if button.type == "dps" and button.roomIndex == roomIndex then
      if (
        gridEntity:GetSaveState().State == 3
        and gridEntity.Position.X == button.pos.X
        and gridEntity.Position.Y == button.pos.Y
