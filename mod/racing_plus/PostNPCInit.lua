...
function PostNPCInit:TheHaunt(npc)
  -- Local variables
  local gameFrameCount = g.g:GetFrameCount()

  -- Speed up the first Lil' Haunt attached to a Haunt (2/3)
  if (
    npc.Variant == 10 -- Lil' Haunt (260.10)
    and npc.Parent ~= nil
  ) then
    ...
    -- NpcState.STATE_MOVE (4), but it produces cleaner results when you go from 3 to 4 rather than
    -- from 0 to 4
    npc.State = NpcState.STATE_IDLE -- 3
    Isaac.DebugString(
      "Manually set a Lil' Haunt to an idle state (3) on frame: " .. tostring(gameFrameCount)
    )
