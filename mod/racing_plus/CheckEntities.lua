...
function CheckEntities:Grid()
  ...
  for i = 1, gridSize do
    ...
    if gridEntity ~= nil then
      ...
      if (
        saveState.Type == GridEntityType.GRID_TRAPDOOR -- 17
        and saveState.VarData == 1 -- Void Portals have a VarData of 1
        and (stage ~= 11 or roomIndex ~= startingRoomIndex)
      ) then
...
CheckEntities.functions[260] = function(entity)
  ...
  local index = GetPtrHash(npc) -- It is safer to use "GetPtrHash()" than "npc.Index"
  if g.run.currentLilHaunts[index] == nil then
    -- This can't be in the NPC_UPDATE callback because it does not fire during the "Appear"
    -- animation
    -- This can't be in the MC_POST_NPC_INIT callback because the position is always equal to (0, 0)
    -- there
  ...
  if (
    ...
    Isaac.DebugString(
      "Removed invulnerability frames and set collision for a Lil' Haunt with index: "
      .. tostring(npc.Index)
    )
  end
