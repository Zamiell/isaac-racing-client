...
function CheckEntities:NonGrid()
  ...
  if CheckEntities.fireworkActive then
    if g.sfx:IsPlaying(SoundEffect.SOUND_BOSS1_EXPLOSIONS) then -- 182
...
function CheckEntities:Entity1000(effect)
  ...
  elseif effect.Variant == EffectVariant.DICE_FLOOR then -- 76
    ...
    if not g.run.diceRoomActivated and
       ...
       g:InsideSquare(g.p.Position, effect.Position, 75) then -- Determined through trial and error
...
function CheckEntities:EntityRaceTrophy(entity)
  ...
  if not g:InsideSquare(g.p.Position, entity.Position, 24) then -- 25 is a touch too big
    return
  end
