...
local core = {}
local api = require("racing_plus/bossapi")
...
local function apiStart()
  api = InfinityBossAPI
  api.AddBossToPool("gfx/drfetusboss/portrait_drfetus.png", "gfx/drfetusboss/bossname_drfetus.png",
                    drFetusType, 0, 0, LevelStage.STAGE2_1, nil, 15, nil, nil, nil)
  api.AddBossToPool("gfx/drfetusboss/portrait_drfetus.png", "gfx/drfetusboss/bossname_drfetus.png",
                    drFetusType, 0, 0, LevelStage.STAGE2_2, nil, 15, nil, nil, nil)
...
function JrFetus:UpdateDrFetus(entity)
  ...
  local isIdle = not api.IsPlaying(sprite, "FlipOff", "AttackUp", "AttackRight", "AttackDown",
                   "AttackLeft", "Slam", "SwimUp", "IdleTop", "AttackTop",
                   "SwimDown", "Appear", "Death") and not activeAttack
    ...
    elseif activeAttack.Name == "CircleShot" then
      if sprite:IsEventTriggered("Slam") then
        ...
        for i = 0, numShots - 1 do
          ...
          core.launchMissile(entity.Position, direction * (entity.Position:Distance(target.Position) * 0.05),
                             entity, fiveShotParams)
        end
    ...
    elseif activeAttack.Name == "GoUpTop" then
      if sprite:IsPlaying("IdleTop") and api.Random(1, 13) == 1 then
        if api.Random(1, 100) > 30 then
          ...
          api.PlaySound(sounds.Whoosh)
          local direction = target.Position - entity.Position
          local bomb = Isaac.Spawn(EntityType.ENTITY_BOMBDROP, BombVariant.BOMB_TROLL, 0,
                                   entity.Position, direction * 0.1, entity)
      ...
      if sprite:IsEventTriggered("Splash") then
        ...
        local offsetEnt = Isaac.Spawn(EntityType.ENTITY_FLY, 0, 0,
                                      entity.Position + topOfTheJarOffset, api.ZeroVector, nil):ToNPC()
