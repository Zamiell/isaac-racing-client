...
function RPPreUseItem:Item124()
  -- Local variables
  local game = Game()
  local player = game:GetPlayer(0)

  RPGlobals.RNGCounter.DeadSeaScrolls = RPGlobals:IncrementRNG(RPGlobals.RNGCounter.DeadSeaScrolls)
  math.randomseed(RPGlobals.RNGCounter.DeadSeaScrolls)
...
function RPPreUseItem:PreventItemPedestalEffects(itemID)
  -- Local variables
  local game = Game()
  ...
  if RPPreUseItem:UnreplacedItemsExist() then
...
function RPPreUseItem:UnreplacedItemsExist()
  -- Local variables
  local game = Game()
  local level = game:GetLevel()
