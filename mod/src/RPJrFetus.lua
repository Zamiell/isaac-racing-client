...
function RPJrFetus:DrFetusEmbryoTakeDamage(entity, amount, flags, source, countdown)
    if entity.Variant == drFetusEmbryoVariant and (entity.HitPoints - amount) < 0 then
        --[[
        if api.Random(1, 10) == 1 then
            local item = api.Random(1, 2)
            if item == 1 then
                Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, CollectibleType.COLLECTIBLE_DR_FETUS, entity.Position, RandomVector() * 3, nil)
            else
                Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, CollectibleType.COLLECTIBLE_EPIC_FETUS, entity.Position, RandomVector() * 3, nil)
            end
        else
        --]]
