...
function RPJrFetus:UpdateDrFetus(entity)
  ...
  local isIdle = not api.IsPlaying(sprite, "FlipOff", "AttackUp", "AttackRight", "AttackDown", "AttackLeft", "Slam",
                                   "SwimUp", "IdleTop", "AttackTop", "SwimDown", "Appear", "Death") and not activeAttack
  ...
  if ai:IsCooledDown() and not activeAttack and isIdle then
    ...
    if not ai:GetBackgroundAttack("FollowingMissile") and
       not ai:GetBackgroundAttack("Pattern") and
       hp < maxhp * 0.5 then
  ...
  elseif activeAttack then
    if activeAttack.Name == "4Shot" then
      if not activeAttack.Firing and
         activeAttack.NumFired < 4 and
         not api.IsPlaying(sprite, "AttackUp", "AttackDown", "AttackLeft", "AttackRight") then
      ...
      elseif not activeAttack.Firing and
             activeAttack.NumFired >= 4 and
             not api.IsPlaying(sprite, "AttackUp", "AttackDown", "AttackLeft", "AttackRight") then
    ...
    elseif activeAttack.Name == "CircleShot" then
      if sprite:IsEventTriggered("Slam") then
        ...
        for i = 0, numShots - 1 do
          ...
          core.launchMissile(entity.Position, direction * (entity.Position:Distance(target.Position) * 0.05), entity,
                             fiveShotParams)
        end
    ...
    elseif activeAttack.Name == "GoUpTop" then
      if sprite:IsPlaying("IdleTop") and api.Random(1, 13) == 1 then
        if api.Random(1, 100) > 30 then
          ...
          api.PlaySound(sounds.Whoosh)
          local direction = target.Position - entity.Position
          local bomb = Isaac.Spawn(EntityType.ENTITY_BOMBDROP, BombVariant.BOMB_TROLL, 0, entity.Position,
                                   direction * 0.1, entity)
      ...
      if sprite:IsEventTriggered("Splash") then
        ...
        local offsetEnt = Isaac.Spawn(EntityType.ENTITY_FLY, 0, 0, entity.Position + topOfTheJarOffset,
                                      api.ZeroVector, nil):ToNPC()
        offsetEnt:FireBossProjectiles(api.Random(18, 26), zeroVector, 10, splashProjectileParams)
        offsetEnt:Remove()
      end

      if not sprite:IsPlaying("SwimUp") and
         not sprite:IsPlaying("IdleTop") and
         not sprite:IsPlaying("AttackTop") and
         not sprite:IsPlaying("SwimDown") then
...
function RPJrFetus:DrFetusTakeDamage(entity, amount, flags, source, countdown)
...
function RPJrFetus:DrFetusEmbryoKill(entity)
...
function RPJrFetus:UpdateMissileTarget(entity)
...
function RPJrFetus:PostNewRoom()
  -- Local variables
...
function RPJrFetus:PostGameStarted()
  -- Local variables
  local challenge = Isaac.GetChallenge()
