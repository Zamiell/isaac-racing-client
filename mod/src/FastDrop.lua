...
-- 3 is fading out
FastDrop.challengeState = 0
FastDrop.challengeFramePressed = 0
...
function FastDrop:PostGameStarted()
  -- Local variables
  local game = Game()
  local room = game:GetRoom()
  local player = game:GetPlayer(0)
...
function FastDrop:PostRender()
  ...
  if FastDrop.challengeState == 2 then
    FastDrop.challengeState = 3
    game:Fadeout(0.05, g.FadeoutTarget.FADEOUT_MAIN_MENU)
  elseif FastDrop.challengeState == 3 then
    return
  end

  -- Wait a moment if they just set a hotkey
  if FastDrop.challengeFramePressed + 10 >= gameFrameCount then
    return
  end

  local text = {}
  if FastDrop.challengeState == 0 then
    if g.race.hotkeyDrop == 0 then
      text[1] = "The custom fast-drop hotkey is not bound."
    else
      text[1] = "The custom fast-drop hotkey is currently bound to:"
      text[2] = FastDrop:GetKeyName(g.race.hotkeyDrop) ..
                " (code: " .. tostring(g.race.hotkeyDrop) .. ")"
    end
  elseif FastDrop.challengeState == 1 then
    if g.race.hotkeySwitch == 0 then
      ...
      text[2] = FastDrop:GetKeyName(g.race.hotkeySwitch) ..
    ...
    for k, v in pairs(Keyboard) do
      if Input.IsButtonPressed(v, i) then
        ...
        elseif FastDrop.challengeState == 1 then
          ...
          FastDrop.challengeState = 2
        end
        SaveDat:Save()
        FastDrop.challengeFramePressed = gameFrameCount
      end
    end
  end
end

function FastDrop:GetKeyName(keyCode)
  for k, v in pairs(Keyboard) do
    if v == keyCode then
      return k:sub(5)
    end
