...
function CheckLoop:Main()
  ...
  -- (this is the index for the Shapes table)
  local floorNum
  ...
  for i = 0, rooms.Size - 1 do -- This is 0 indexed
    ...
    if roomType == RoomType.ROOM_DEFAULT then -- 5
      ...
      if roomIndex == startingRoomIndex then
        ...
        -- The starting room is in "00.special rooms.stb", so it may not be in the Shapes table
        -- So, manually assign it the correct shape
        roomShape = RoomShape.ROOMSHAPE_1x1 -- 1
      else
        roomShape = Shapes[floorString][roomDataVariant]
      end
      ...
      local x, y = CheckLoop:GetXYFromGridIndex(roomIndex)
      grid[y][x] = i
  ...
  for i, roomData in pairs(roomsData) do
    ...
    local adjacentSquares = CheckLoop:GetAdjacentSquares(roomData.roomShape)
    ...
    CheckLoop.nodes[i] = connectedRooms
  end

  --[[
  -- Print out the connection list
  Isaac.DebugString("Room connection list:")
  for i, node in pairs(CheckLoop.nodes) do
  ...
  CheckLoop.visited = {}
  return CheckLoop:HasCycle(startingRoomNum, CheckLoop.nodes)
end

-- Get the grid coordinates on a 13x13 grid
function CheckLoop:GetXYFromGridIndex(idx)
  -- 0 --> (0, 0)
  -- 1 --> (1, 0)
  -- 13 --> (0, 1)
  -- 14 --> (1, 1)
...
function CheckLoop:GetAdjacentSquares(roomShape)
...
function CheckLoop:HasCycle(node, cameFrom)
  -- If we found this node already, there is a cycle
  for i = 1, #CheckLoop.visited do
    if CheckLoop.visited[i] == node then
  ...
  CheckLoop.visited[#CheckLoop.visited + 1] = node

  -- Go through all the nodes that are connected to this node
  for _, n in ipairs(CheckLoop.nodes[node]) do
    if n ~= cameFrom then
      if CheckLoop:HasCycle(n, node) then
        return true
      end
    end
  end
...
-- to see if there is a cycle (loop) in the node connection list
function CheckLoop:HasCycle2(node, cameFrom)
