...
function onMessage(message: string) {
  ...
  // when to die. We need to stay alive until authentication is over, but killed after that so we it
  // will not interfere with launching Isaac. (Greenworks uses the same AppID as Isaac, so Steam
  // gets confused.)
  if (message === "exit") {
    process.exit();
  }
...
function greenworksInit() {
  ...
  // C:\Users\[Username]\AppData\Local\Programs\RacingPlus\steam_appid.txt (in production) or
  ...
  // located in the "static" directory. The current working directory in development is:
  // D:\Repositories\isaac-racing-client\dist\main\childProcesses The current working directory in
  // production is:
  ...
  // Initialize Greenworks. (This cannot be written as `!greenworks.init()`.)
...
function successCallback(
  ...
  // The ticket will become invalid if the process ends. Thus, we need to keep the process alive
  // doing nothing until we get a message that the authentication is over.
}
