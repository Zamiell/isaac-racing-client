...
PARSER.add_argument(
    "-s", "--skipmod", help="skip all mod related stuff", action="store_true"
)
PARSER.add_argument(
    "-m", "--mod", help="only do mod related stuff", action="store_true"
)
...
if not ARGS.skipmod:
    ...
    LARGE_FONT = ImageFont.truetype(
        os.path.join("src", "fonts", "Jelly Crazies.ttf"), 9
    )
    SMALL_FONT = ImageFont.truetype(
        os.path.join("src", "fonts", "Jelly Crazies.ttf"), 6
    )
    ...
    for file_name in os.listdir(ROOMS_DIR):
        if file_name.endswith("2.stb"):
            ...
            os.rename(
                os.path.join(ROOMS_DIR, file_name),
                os.path.join(ROOMS_DIR, new_file_name),
            )
    ...
    for root, subdirs, files in os.walk(MOD_DIR):
        for fpath in [os.path.join(root, f) for f in files]:
            ...
            if (
                name == "metadata.xml"
                or name  # This file will be one version number ahead of the one distributed through Steam
                == "save1.dat"
                or name  # These are the IPC files, so it doesn't matter if they are different
                == "save2.dat"
                or name == "save3.dat"
            ):
    ...
    if os.path.exists(MOD_DIR2):
        try:
            subprocess.call(
                ["rm", "-rf", MOD_DIR2]
            )  # Works on Windows if "GnuWinCoreutils-5.3.0.exe" is installed
        except Exception as err:
...
if ARGS.github:
    ...
    # http://stackoverflow.com/questions/17140886/how-to-search-and-replace-text-in-a-file-using-python
    GLOBALS_FILE = os.path.join("src", "js", "g.js")
